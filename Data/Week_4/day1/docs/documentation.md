...
```
--Here I consider staff whose punch in time is 18:00:00 , is comming for night shift

with cte_hours as (
select case when EXTRACT(hours from tr.punch_in_time)>18 THEN 'Night'
		else 'OTHER'
		end as e,
		t.shift_type
...
)select case when e=shift_type THEN 'Correct'
		else 'Incorrect'
		end as label,COUNT(*)
from cte_hours
where e = 'Night'
